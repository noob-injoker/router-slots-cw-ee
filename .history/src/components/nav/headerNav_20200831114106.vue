<template>
<div class="df h00  j-bettween a-center">
  <div class="m-l-20">欢迎来到小艾后台管理系统</div>
  <div class="df">

    <div v-if="date">{{year}}年{{month}}月{{day}}日,上午<span>{{hours}}</span>:<span>{{minuts}}</span>:<span>{{secondes}}</span></div>

    <div class="wether">天气</div>
    <div class="topUsername-box">亲爱的,{{topUsername}}</div>
    <div class="logOut" @click="logOut"><a href="#">退出</a></div>
